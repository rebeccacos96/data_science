...
kitchen_sink_regression <- lm(y ~ ., data = simulated) %>%
  tidy() #%>% filter(term == 'd')
...
## Naive Lasso
Now let's try to use Lasso and cross-validation to select variables that explain y. Lasso helps us with model selection, Ridge just shrinks the coefficient to almost zero.
...
simulated %>%
  select(y, d, all_of(Lasso_coefficients$coef_names[-1])) %>%
  lm(y ~ ., data = .) %>%
  tidy()
```
V3 is missing, because it is very correlated to d.
