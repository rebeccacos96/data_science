...
***
dplyr (tidyverse) - allows us to work with data in a dataframe (everything that is not a matrix)
...
dataset %>% group_by(Female, ethnicity) %>% 
  summarise(mean_priors = mean(Number_of_Priors),
            n = n())
```
...
a way to reverse the gather() function: spread(key (var1), value(var2))
...
You can quickly visualize penalties by using the basic `plot()` function. At the beginning the penalty is high and then goes low.
...
Let's create matrix `X` and vector `y`. Number of priors is removed as it is our predictor variable.
...
* (Note: `map()` function comes from [purrr](https://purrr.tidyverse.org/) package). It is useful to avoid looping.

dbl - just want a vector


```{r l2norm}
l2 <- map_dbl(.x = c(1:100), ~sqrt(sum(coef(ridge_reg)[-1,.]^2)))
plot(x = log(grid), y = l2)
```

Explain the above relationship:
Ridge regression: high penalty on the sum of squares of the coefficients. Higher the lambda, smaller the beta (inverse relationship). 
...
relationship between lambda and OLS and lambda and intercept: if lambda is a very high value, it is like fitting using only an intercept to predict. The opposite is true for OLS
```{r testMSE_intercept}
ridge_pred <- predict(ridge_reg, s = 10^10, newx = X[test, ])
cat("For lambda = 10^10, the test MSE equals to ", mean((ridge_pred - y_test)^2))
...
cat("For lambda = 0, the test MSE equals to ", mean((ridge_pred - y_test)^2))
```
...
Interpreting this result: It provides intuition of the confidence interval outside of the coefficients. 0.88 is the standard error around the predicted coefficient.
